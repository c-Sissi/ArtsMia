...
public class ArtsmiaDAO {
	...
	public void listObjects(Map<Integer,ArtObject> objIdMap) {
		...
		try {
			...
			while (res.next()) {
				...
				if(!objIdMap.containsKey(res.getInt("object_id"))) {
					ArtObject artObj = new ArtObject(res.getInt("object_id"), res.getString("classification"), res.getString("continent"), 
							res.getString("country"), res.getInt("curator_approved"), res.getString("dated"), res.getString("department"), 
							res.getString("medium"), res.getString("nationality"), res.getString("object_name"), res.getInt("restricted"), 
							res.getString("rights_type"), res.getString("role"), res.getString("room"), res.getString("style"), res.getString("title"));
					
					objIdMap.put(artObj.getId(), artObj);
				}
			}
			conn.close();
	...
	public int getPeso(ArtObject a1, ArtObject a2) {
		...
		String sql = "SELECT e2.object_id, COUNT(*) AS peso "
				...
				+ "WHERE e1.exhibition_id = e2.exhibition_id AND e1.object_id = ? AND e1.object_id != e2.object_id  ";
	...
	public List<Adiacenze> getAdiacenze(Map<Integer,ArtObject> objIdMap){
		...
		try {
			...
			ResultSet res = st.executeQuery();
			while(res.next()) {
				result.add(new Adiacenze(objIdMap.get(res.getInt("e1")),objIdMap.get(res.getInt("e2")),res.getInt("peso")));
